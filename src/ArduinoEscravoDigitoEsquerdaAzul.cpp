...
void loop() {
  if (Serial.available() > 0) {
    ...
    // Interpretar a mensagem e extrair a pontuação do time azul
    teamScore = extractTeamScore(receivedMessage, "Azul");
    // Exibir o dígito mais à direita da pontuação do time azul
    displayDigit((teamScore >= 10) ? (teamScore / 10) : 0); // Se a pontuação for menor que 10, exibe 0
